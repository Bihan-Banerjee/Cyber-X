/**
 * defenderActionMap.ts — the bridge between the trained RL defender and CyberX.
 *
 ...
 *
 * NOTE (paper): the `d3fend` names below map to real MITRE D3FEND techniques and
 * double as the action↔technique grounding table (Phase D). Verify the exact
 * technique IDs against d3fend.mitre.org before citing them in the paper.
 */
...
export interface DefenderActionInfo {
  ...
  description: string;
  /** corresponding MITRE D3FEND defensive technique (name) */
  d3fend: string;
...
/**
 * Indexed by action id (0–11), in the exact order of DEF_ACTION_NAMES so a
...
/** Look up a defender action by its policy id (0–11) or canonical name. */
