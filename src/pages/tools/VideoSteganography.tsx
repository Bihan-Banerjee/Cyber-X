...
import { Loader2, AlertCircle, Upload, X, Video, Eye, EyeOff, Download, Copy, Check } from "lucide-react";
...
const VideoSteganography = () => {
  ...
  const [stegoVideo, setStegoVideo] = useState<File | null>(null);
  ...
  const handleCoverVideoSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleStegoVideoSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      if (!file.type.startsWith('video/')) {
        setError('Please upload a valid video file');
  ...
  const handleHideData = async () => {
    if (!coverVideo || !secretMessage.trim()) return;
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/scan/video-stego-hide`, {
        method: "POST",
        body: formData,
      });
      ...
      setError(err.message || "Failed to hide data in video");
    } finally {
  ...
  const handleExtractData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/scan/video-stego-extract`, {
        method: "POST",
        body: formData,
      });
      ...
      setError(err.message || "Failed to extract data from video");
    } finally {
      setIsProcessing(false);
    }
  };

  const downloadStegoVideo = () => {
    if (!hideResult?.outputVideo) return;

    const link = document.createElement('a');
    link.href = hideResult.outputVideo;
    link.download = hideResult.videoName;
    link.click();
  };
  ...
  return (
    <CyberpunkCard title="VIDEO STEGANOGRAPHY">
      <div className="space-y-6">
        ...
        {mode === "hide" ? (
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={handleCoverVideoSelect}
                  className="hidden"
                  id="cover-video-input"
                />
                <label
                ...
                >
                  ...
                  <div className="text-center">
                    {coverVideo ? (
                      <>
                        <p className="text-cyber-cyan text-sm font-semibold">{coverVideo.name}</p>
                        <p className="text-xs text-gray-500">{(coverVideo.size / 1024 / 1024).toFixed(2)} MB</p>
                      </>
                    ) : (
                      <>
                        <p className="text-cyber-cyan text-sm font-semibold">Upload cover video</p>
                        <p className="text-xs text-gray-500">MP4, WebM, OGG, AVI</p>
                      </>
                    )}
                  </div>
                </label>
                {coverVideo && (
                  <button
                    onClick={() => { setCoverVideo(null); setCoverPreview(null); }}
            ...
            {coverPreview && (
              <div className="glass-panel rounded p-4">
                <h3 className="text-sm font-bold text-cyber-cyan mb-2">Cover Video Preview</h3>
                <video controls src={coverPreview} className="w-full rounded" style={{ maxHeight: '300px' }} />
              </div>
            )}
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Enter the secret message to hide in video..."
            ...
            <Button
              ...
              disabled={isProcessing || !coverVideo || !secretMessage.trim()}
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-cyber-cyan mb-2 tracking-wide">
                STEGO VIDEO (Video with hidden data)
              </label>
              <div className="flex gap-2">
                <input
                  ref={stegoInputRef}
                  type="file"
                  accept="video/mp4,video/webm,video/ogg,video/avi"
                  onChange={handleStegoVideoSelect}
                  className="hidden"
                  id="stego-video-input"
                />
                <label
                  htmlFor="stego-video-input"
                  className="flex-1 flex items-center justify-center gap-2 p-4 bg-black/50 border-2 border-dashed border-cyber-cyan/30 rounded cursor-pointer hover:border-cyber-cyan/50 transition-colors"
                >
                  <Upload className="w-5 h-5 text-cyber-cyan" />
                  <div className="text-center">
                    {stegoVideo ? (
                      <>
                        <p className="text-cyber-cyan text-sm font-semibold">{stegoVideo.name}</p>
                        <p className="text-xs text-gray-500">{(stegoVideo.size / 1024 / 1024).toFixed(2)} MB</p>
                      </>
                    ) : (
                      <>
                        <p className="text-cyber-cyan text-sm font-semibold">Upload stego video</p>
                        <p className="text-xs text-gray-500">MP4, WebM, OGG, AVI</p>
                      </>
                    )}
                  </div>
                ...
                {stegoVideo && (
                  <button
                    onClick={() => { setStegoVideo(null); setStegoPreview(null); }}
                    className="p-3 bg-red-500/20 hover:bg-red-500/30 border border-red-500/50 rounded transition-colors"
                  >
                    <X className="w-5 h-5 text-red-400" />
                  </button>
            ...
            {stegoPreview && (
              <div className="glass-panel rounded p-4">
                <h3 className="text-sm font-bold text-cyber-cyan mb-2">Stego Video Preview</h3>
                <video controls src={stegoPreview} className="w-full rounded" style={{ maxHeight: '300px' }} />
              </div>
            )}
            ...
            <Button
              ...
              disabled={isProcessing || !stegoVideo}
              className="w-full bg-cyber-red hover:bg-cyber-deepRed text-white font-bold tracking-wider"
            >
              {isProcessing ? (
                <>
        ...
        <div className="text-xs text-gray-500 p-3 bg-black/30 rounded">
          <p className="font-semibold text-cyber-cyan mb-1">ℹ️ Video Steganography</p>
          <p>Hides data in video files using metadata embedding technique. The hidden data is imperceptible and preserves video quality. Works with MP4, WebM, OGG, and AVI formats. Optional AES encryption for enhanced security.</p>
        </div>
            ...
            <div className="glass-panel rounded p-6">
              ...
              <div className="grid md:grid-cols-4 gap-3 mb-4">
                ...
                <div className="p-3 bg-cyber-cyan/10 border border-cyber-cyan/30 rounded text-center">
                  ...
                  <p className="text-xs text-cyber-cyan">Capacity</p>
                </div>
                <div className="p-3 bg-yellow-500/10 border border-yellow-500/30 rounded text-center">
                  <p className="text-sm font-bold text-yellow-400">{hideResult.technique}</p>
                  <p className="text-xs text-yellow-400">Technique</p>
                </div>
              ...
              <div className="mb-4 p-4 bg-black/30 rounded">
                <div className="flex items-center gap-2">
                  <Video className="w-5 h-5 text-cyber-cyan" />
                  <div>
                    <p className="text-sm font-bold text-cyber-cyan">{hideResult.videoName}</p>
                    <p className="text-xs text-gray-400">Stego video ready for download</p>
                  </div>
                </div>
              </div>

              <Button
                onClick={downloadStegoVideo}
            ...
            <div className="glass-panel rounded p-6">
              ...
              <div className="grid md:grid-cols-3 gap-3 mb-4">
                ...
                <div className="p-3 bg-cyan-500/10 border border-cyan-500/30 rounded text-center">
                  <p className="text-sm font-bold text-cyan-400">{extractResult.technique}</p>
                  <p className="text-xs text-cyan-400">Technique</p>
                </div>
                <div className="p-3 bg-purple-500/10 border border-purple-500/30 rounded text-center">
